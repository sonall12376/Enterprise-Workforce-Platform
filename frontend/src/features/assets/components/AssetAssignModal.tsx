...
export const AssetAssignModal: React.FC<AssetAssignModalProps> = ({ asset, isOpen, onClose, onAssign, isLoading }) => {
  ...
  useEffect(() => {
    const loadEmployees = async () => {
      ...
      setFetchError(null);
      try {
        // Reuse taskService's organizational members endpoint
        ...
        setFetchError('Failed to fetch eligible employees');
      } finally {
    ...
    <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-md w-full overflow-hidden shadow-2xl relative flex flex-col">
        ...
        <div className="flex justify-between items-center px-6 py-4 border-b border-slate-800/80">
          <h2 className="text-xl font-bold text-slate-100 flex items-center gap-2">Allocate Asset</h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div className="text-sm text-slate-400 bg-slate-850/40 p-4 rounded-xl border border-slate-800/80 space-y-1">
            <p>
              <span className="text-slate-500">Asset:</span> <strong className="text-slate-200">{asset.name}</strong>
            </p>
            <p>
              <span className="text-slate-500">Serial ID:</span>{' '}
              <code className="text-violet-400 text-xs">{asset.serialNumber}</code>
            </p>
            <p>
              <span className="text-slate-500">Type:</span> <span className="text-slate-300">{asset.type}</span>
            </p>
          </div>
          ...
          <div className="space-y-1.5">
            ...
            {fetchingEmployees ? (
              ...
              <p className="text-xs text-slate-500 italic">No employees available for assignment</p>
            )}
          </div>
